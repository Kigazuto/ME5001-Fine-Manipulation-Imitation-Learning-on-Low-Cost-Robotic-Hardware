...
class ManipulatorRobot:
    ...
    def connect(self):
        ...
        # Wait for motors to stabilize after torque activation
        import time
        time.sleep(1)
    ...
    def _safe_read(self, arm, data_name):
        """Read with fallback to last known value on ConnectionError."""
        if not hasattr(self, '_last_read'):
            self._last_read = {}
        key = f"{id(arm)}_{data_name}"
        try:
            val = arm.read(data_name)
            self._last_read[key] = val
            return val
        except ConnectionError:
            if key in self._last_read:
    ...
    def teleop_step(
        ...
        # Initialize smoothing buffer on first call
        if not hasattr(self, '_smooth_pos'):
            self._smooth_pos = {}
        ...
        for name in self.leader_arms:
            ...
            leader_pos[name] = self._safe_read(self.leader_arms[name], "Present_Position")
        ...
        for name in self.follower_arms:
            ...
            if self.config.max_relative_target is not None:
                present_pos = self._safe_read(self.follower_arms[name], "Present_Position")
                present_pos = torch.from_numpy(present_pos)
                goal_pos = ensure_safe_goal_position(goal_pos, present_pos, self.config.max_relative_target)

            # Smooth goal position with EMA (alpha=0.6: responsive but smooth)
            if name not in self._smooth_pos:
                self._smooth_pos[name] = goal_pos.clone()
            else:
                alpha = 0.4
                self._smooth_pos[name] = alpha * goal_pos + (1 - alpha) * self._smooth_pos[name]
            goal_pos = self._smooth_pos[name]
        ...
        for name in self.follower_arms:
            ...
            follower_pos[name] = self._safe_read(self.follower_arms[name], "Present_Position")
